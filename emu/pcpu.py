...
class CPU:
    ...
    def __init__(self, mmu=None, pc=None, stack_page=0x1, magic=0xee, logging=False):
        """
        Parameters
        ----------
    ...
    def step(self):
        ...
        if self.logging:
            ...
            for op in self._ops:
                for route in op[2]:
                    if opcode == route[2][0]:
                        print("%s (%s) @ %s a %s x %s y %s sp %s mb %s" % (op[0],hex(opcode),hex(current_pc),hex(self.r.a),hex(self.r.x),hex(self.r.y),hex(self.r.s),hex(self.mmu.ram_pointer)))
                        found_op = True
                        break
                if found_op:
                    break
    ...
    def interrupt(self,name):
        if name == "BRK":
            ...
            self.stackPushWord(self.r.pc)
        self.stackPush(self.r.p)
        self.r.setFlag('I')
        self.r.pc = self.interruptAddress(name)
    ...
    def RTS(self, _):
        address = (self.stackPopWord() + 1) & 0xffff
        if self.logging:
            print("return",hex(address))
        self.r.pc = address
